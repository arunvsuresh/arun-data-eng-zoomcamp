...
@flow()
def load_local_to_gcs(months, year, color):
    etl_parent_flow(months, year, color)

@task(retries=3, log_prints=True)
def extract_from_gcs(color: str, year: int, months: list[int]) -> list[Path]:
...
def etl_gcs_to_bq(months: list[int] = [1, 2],
    ...
    # load_local_to_gcs(months, year, color)
